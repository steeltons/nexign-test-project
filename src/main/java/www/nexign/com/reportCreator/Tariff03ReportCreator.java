...
public class Tariff03ReportCreator implements ReportCreator{

	private final double MINUTES_COST = 1.5;
	
	Tariff03ReportCreator() {}
	...
	private List<PhoneReport> createPhoneReports(List<CallInformation> callInfoList) {
		List<PhoneReport> phoneReportList = new ArrayList<>();
		for(CallInformation callInfo : callInfoList) {
			long callDurationSeconds = callInfo.getCallDurationInSeconds();
			double price = Math.ceil(callDurationSeconds / 60.0) * MINUTES_COST;
